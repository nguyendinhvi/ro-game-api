...
function parseRelatedQuery(query: Record<string, unknown>): IRelatedGamesQuery {
  const limitRaw = query.limit ? parseInt(String(query.limit), 10) : undefined;
  const result: IRelatedGamesQuery = {};
  if (Number.isInteger(limitRaw) && (limitRaw as number) > 0) {
    result.limit = limitRaw as number;
  }
  return result;
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const typedReq: GameRequest<unknown, unknown, IGameQuery> = Object.assign(
      req,
      {
        query: parsed,
      },
...
router.get(
  "/slug/:slug/related",
  asyncHandler(async (req, res) => {
    const parsed = parseRelatedQuery(req.query as Record<string, unknown>);
    await gameController.getRelated(
      Object.assign(req, { query: parsed }) as unknown as GameRequest<
        unknown,
        { slug: string },
        IRelatedGamesQuery
      >,
